...
# Endpoint to retrieve Grafana dashboard
@app.get("/grafana-dashboard")
@admin_required()
async def get_grafana_dashboard(request: Request):
    """Retrieve a Grafana dashboard using Grafana's API."""
    grafana_url = "http://localhost:3000/api/dashboards/uid/{dashboard_uid}"
    headers = {
        'Authorization': 'Bearer YOUR_GRAFANA_API_KEY'  # Замените на ваш реальный API-ключ
    }
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve Grafana dashboard: {e}")
